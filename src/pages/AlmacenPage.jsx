...
const AREAS = INVENTORY_AREAS.filter((a) => a.group === 'almacen')
...
export function AlmacenPage() {
  ...
  if (!canView) {
    return (
      <div className="card pad">
        <h1 className="card__title">Almacén</h1>
        <p className="muted">No tienes permiso para ver ninguna sección de almacén.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <ModuleHeader
        title="Almacén"
        lead="Guías de despacho, palés, stock de almacén y recepción de mercadería según tu rol."
      />

      <ModuleTabs tabs={allowedAreas} activeId={areaTab} onChange={setAreaTab} ariaLabel="Áreas de almacén" />
      ...
      {areaTab === 'guias' && canViewGuias ? (
        ...
        <CantosCatalogPanel />
      ) : areaTab === 'rm' && canViewRm ? (
        <>
          <ModuleHeader title="Recepción de mercadería">
            {guiaIndexErr ? (
          ...
          <ModuleTabs tabs={TABS} activeId={tab} onChange={setTab} ariaLabel="Tipo de registro RM" />
